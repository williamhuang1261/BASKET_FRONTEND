      ...
      <div className="flex h-full w-full flex-col items-center rounded-b border-x border-b border-gray-200 bg-white px-2 inset-shadow-sm">
        ...
        >
          <p className="text-sm font-semibold text-gray-500">
            Profitez de notre nouveau programme de fidélité!
          </p>
...
const ItemDetailBoxMain = ({ image }: Props) => {
  return (
    <div className="flex items-center justify-between rounded-sm border border-gray-200 p-2 shadow">
      <div className="h-full">
        <h3 className="flex w-full items-center justify-center text-lg font-bold">
          Autre
        </h3>
        <img src={image || BasketLogo} className="w-24 object-contain" />
        <StarReviews rating={2} />
      </div>
      <div className="h-full max-w-1/3">
        <div className="flex items-end gap-2">
          <span className="font-righteous text-2xl text-black">
            2.99 $ / lb
          </span>
...
const ItemDetailBox = ({ image }: Props) => {
  return (
    <div className="">
      <ItemDetailBoxMain image={image} />
    </div>
  );
};
