    ...
    <div className="flex flex-col py-2">
      ...
      <div className={"overflow-hidden rounded-sm "}>
        {filtered.opts?.slice(1).map((s) => (
          <div
            className={`${open ? "h-10 p-2 opacity-100" : "h-0 p-0 opacity-0"} ${filtered.opts?.indexOf(s) !== 1 ? "border-t-0.5 border-gray-200" : ""} flex items-center justify-between gap-2 transition-all`}
